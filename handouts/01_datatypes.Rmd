...
R uses **functions** (basically, an inbuilt bit of code that you can call on to do things with, optionally passing it some data to woprk with) to take some inputs and get an output. The function that we can pass a value to, and get what data type it is as the output, is the `class()` function.
...
2. What is the datatype of the value returned by `Sys.time()`?
3. What is the datatype of the value returned by `Sys.timezone()`?
4. Make this quote into an R string

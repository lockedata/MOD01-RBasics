...
The following section will show some things that exemplify how little is required to do things in R, that in other languages or tools can take a substantial amount of time.
...
```{block type='future'}
rmarkdown files are for generating documents with text and R code interleaved (like this book!) They'll will be covered in a book in future. If you want to get going, checkout the [rmarkdown](//rmarkdown.rstudio.com) site.
```
...
- Overwrite the `a` with an `A` and go back into the code completion. Do you get the same list? What's different and why?
```
...
Most commonly, you'll want to create a new directory project. Once selected it'll then give you the option to create an empty project, a Shiny project (Shiny is a feature we'll cover in a later book too,) or an R package. You'll normally select the empty projects. Once an option is selected, provide a name and where the project should go.

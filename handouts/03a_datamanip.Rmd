# Data manipulation
## Grid references
With R objects, it's possible to use a grid reference system to select values out of an object. 

In vectors and lists, you can specify the element position as they're only have a single dimension. In data.frames, you can pinpoint both the element (row) and the column.
...
If you want everything in an object, you can just use the object's name or put square brackets after it.
...
Since even a single value is a vector in R, it means we can provide longer vectors in our grid specifications.
...
If you specify an position that doesn't exist, R will return an **NA** (a missing value) for that position.
```{r}
letters[23:29]
```
...
You are able to use element positions and element names in data.frame selections if you stick to one type of reference in each position.
...
If you provide boolean values into our grid reference system it will return elements that have a TRUE associated with it, and ignore values with a FALSE associated with it.
...
If you need to select a given named value or column from an object, there are some alternative selection methods you do.

There are double square brackets for when you expect only, and only one, named element.
```{r}
mylist[["a"]]
```
...
## Changing objects
By utilising our reference systems, not only we can select daata of interest to us, but we can add new data, update existing values, and even delete values.
...
An alternative method is to make a selection and assign them the value NULL. NULL can removes contents in lists and data.frames.
